# fingerprint reverse permutations
...
def match(dataDir = '/home/hallee/scratch/hcp/targets/fingerprint'):
    # create an empty df with cols sub1, correct, cond, roi
    df = pd.read_csv(f'{dataDir}/fingerprint_df_109_reverse.csv', sep=',', header = 0)
...
def shuffle(p, match, dataDir='/home/hallee/scratch/hcp/targets/fingerprint'):
    results = pd.DataFrame(columns = ['sub1', 'correct', 'cond', 'roi'])
    df = pd.read_csv(f'{dataDir}/fingerprint_df_109_reverse.csv', sep=',', header = 0)
    for roi in rois:
...
def export_distributions(results, num, outDir='/home/hallee/scratch/hcp/targets/fingerprint/fingerprint_permutations'):
    for roi in rois:
        df = results[results['roi']==roi]
        dist = df['movie-rest']
        pd_dist = pd.DataFrame(dist)
        pd_dist.to_csv(f'{outDir}/{roi}_dist_{num}_reverse.csv')
        print(f'the 95th percentile cutoff of the {roi} is (reverse):', np.percentile(dist, 95))

#the big giant huge function
