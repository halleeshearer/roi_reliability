...
def rearrange_matlab_movie(tr):
    ...
    for roi in rois:
        ...
        matrix.to_csv(f'/home/hallee/scratch/hcp/targets/data_amount/i2c2/rearranged/rearranged_{roi}_MOVIE_{tr}tr_109.csv', index = False, header = False)
        matrix = pd.DataFrame()

#run
pool = mp.Pool(mp.cpu_count())
results = pool.map(rearrange_matlab_movie, trs)
pool.close()
