# create matrices for each roi (of the three rois), for each subject, for each condition, for each number of TRs
...
# parcels in the Glasser atlas of each ROI
rois = {"dlpfc": [73, 67, 97, 98, 26, 70, 71, 87, 68, 83, 85, 84, 86],
        "pre_sma": [224],
        "tpj": [139, 140, 141, 28, 25]}
...
# for data amount specifically, we need to crop the data to the number of TRs in the index file
...
def create_matrices():
    ...
    for sub in subject_list_n109:
        ...
        for cond in conditions:
            ...
            # loop through number of TRs to calculate matrices for
            for tr in range(20, 700, 20):
                cropped_filtered_parcellated_ts = filtered_parcellated_ts[0:tr,:]
                cropped_filtered_vertex_ts = filtered_vertex_ts[0:tr,:]

                # loop through all of the regions in the Glasser atlas
                for roi in rois: ## TODO: Fix this line to be range(1, hcp.mmp.ids.shape[0]+1) to get roi 380 too! IF we want subcort
                    # create a logical vector that is True for vertices in this roi
                    roi_idx = np.isin(hcp.mmp.map_all, rois[roi]) ## CHECK, START BACK HERE
                    # keep those vertices in this roi
                    roi_vert_ts = cropped_filtered_vertex_ts[:,roi_idx]
                    ...
                    mat = np.corrcoef(roi_vert_ts, cropped_filtered_parcellated_ts, rowvar=False)[roi_vert_ts.shape[1]:,:roi_vert_ts.shape[1]]
                    # export the results
                    np.savetxt(f'/scratch/st-tv01-1/hcp/reliability/matrices/three_rois/data_amount/{str(sub)}/{cond}_{roi}_{tr}TRs.csv', mat, delimiter = ',')
